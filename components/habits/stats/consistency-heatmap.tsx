      ...
      {data.map((day, i) => {
        ...
        return (
          ...
          >
            ...
            />
            {/* Simple CSS tooltip */}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max px-3 py-2 bg-surface-dark text-surface-card text-xs rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 pointer-events-none">
              <p className="font-medium mb-0.5">{day.formattedDate}</p>
              <p className="text-surface-card/80">{day.completed} of {day.total} habits ({day.percentage}%)</p>
            </div>
          </div>
        );
      })}
